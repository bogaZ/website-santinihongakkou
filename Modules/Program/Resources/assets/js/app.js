...
import {
    tinyConfig,
    tinymce,
} from "../../../../../resources/js/vendor/tinyMce.js";
...
$(function () {
    ...
    $(document).on("submit", ".js-form", function (e) {
        ...
        formData.append("content[id]", tinymce.get("content-id").getContent());
        clearError();
        $.ajax({
            type: "POST",
            url: url,
    ...
    $(".modal").on("hidden.bs.modal", function () {
        $(".tiny-default").each(function (e) {
            const selector = `${$(this).attr("id")}`;
            const editor = tinymce.get().find((item) => item.id == selector);
            editor.remove();
        });
    });
